...
import 'package:okra_distributer/FirmPartner/Withdraw/withdraw_bloc/withdraw_event.dart';
import 'package:okra_distributer/FirmPartner/Withdraw/withdraw_bloc/withdraw_state.dart';
...
class _WithdrawState extends State<Withdraw> {
  ...
  Future<void> _submitData() async {
    final patnerFirmId = context.read<WithdrawBloc>().state.selectPatnerFirm;
    final withdrawalAmount = withdrawalController.text;
    final bankFromId = context.read<WithdrawBloc>().state.selectBankForm;
    final bankToId = context.read<WithdrawBloc>().state.bankTo;
    ...
    final data = {
      ...
      'dcWithdrawalAmount': withdrawalAmount,
      'iCommonBankID': bankFromId,
      'iBankID': bankToId,
      'date': date,
    };

    final url = Constant.patnerWithdrawal;
    final response =
    ...
    if (response.statusCode == 200) {
      context.read<WithdrawBloc>().add(ResetAllStates());
      withdrawalController.clear();
      dateController.clear();
      // context.read<WithdrawBloc>().add(SelectPatnerFirm(null));
      // context.read<WithdrawBloc>().add(SelectbankFrom(null));
      // context.read<WithdrawBloc>().add(SelectBankTo(null));
  ...
  void initState() {
    super.initState();
    context.read<WithdrawBloc>().add(FetchPatnerFirm());
    context.read<WithdrawBloc>().add(FetchBankFrom());
    context.read<WithdrawBloc>().add(FetchBankTo());
    context.read<WithdrawBloc>().add(SelectPatnerFirm(null));
    context.read<WithdrawBloc>().add(SelectbankFrom(null));
    context.read<WithdrawBloc>().add(SelectBankTo(null));
  }
                    ...
                    child: Padding(
                      ...
                      child: BlocBuilder<WithdrawBloc, WithdrawState>(
                        builder: (context, state) {
                          ...
                          //     state.selectPatnerFirm;
                          // final selectedValueExists = state.patnerFirmList;
                  ...
                  Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Add Widthdraw amount ',
